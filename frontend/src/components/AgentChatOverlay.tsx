    ...
    <>
      {open && <aside className="agent-chat-panel" aria-label={isZh ? "智能助手" : "Agent chat"}>
        <header className="agent-chat-header">
          ...
          <div className="agent-chat-actions">
            <div className="workspace-menu-anchor">
              <button onClick={() => setHistoryOpen((value) => !value)} aria-label={isZh ? "聊天历史" : "Chat history"} aria-expanded={historyOpen}><History size={17} /></button>
              {historyOpen && <div className="chat-history-popover">
                ...
                <div className="chat-history-list">
                  {sessions.map((session) => <div key={session.id} className={`chat-history-item ${session.id === activeSessionId ? "is-active" : ""}`}>
                    ...
                    <button className="chat-history-delete" onClick={() => onDeleteSession(session.id)} aria-label={isZh ? `删除 ${session.title}` : `Delete ${session.title}`}><Trash2 size={13} /></button>
                  </div>)}
                </div>
              </div>}
            </div>
            <button onClick={onCreateSession} aria-label={isZh ? "新建对话" : "New conversation"}><Plus size={17} /></button>
            <button onClick={() => onOpenChange(false)} aria-label={isZh ? "关闭聊天" : "Close chat"}><X size={17} /></button>
          </div>
        </header>
        ...
        <form className="agent-chat-composer" onSubmit={submit}>
          ...
          <button type="submit" disabled={!input.trim() || !isConnected} aria-label={isZh ? "发送" : "Send"}><Send size={16} /></button>
        </form>
      </aside>}
