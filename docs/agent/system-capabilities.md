...
- Skill Registry：已安装 Instruction Skill 的 metadata、版本、digest、enabled 和 availability。
...
每个可执行条目至少包含稳定 ID、说明、availability、effect、机器可读的 `scope_contract`、approval 要求和限制。`scope_contract` 同时声明 required/optional 字段、类型、枚举或数值边界及一个最小合法示例；Schema Alignment Agent 不需要从字段名猜测嵌套结构。Skill 条目只包含全局可投影 metadata，不包含 `SKILL.md` 正文。Secret、凭据、绝对路径、完整 Graph 记录和无界历史绝不进入 Catalog。
...
| 角色 | 获得的内容 | 不获得的内容 |
| --- | --- | --- |
| Intent Router | 可选 intent、已有 App 摘要、Graph schema、能力是否存在、已安装 Skill metadata 与 availability | Controller 源码、Skill 正文、凭据、任意 adapter 内部细节 |
| Converse Agent | 只读 tool schema、Graph/App 摘要、Run 行为、当前 Run 固定的相关 Skill 正文 | 写 tool、未选中的 Skill 正文、未批准 Widget grant、直接发布能力 |
...
Skill 正文不属于 `SystemCapabilityCatalog`。Router 只用 metadata 判断相关性；选中后，Skill loader 从内容寻址 snapshot 即时读取正文，并把 `catalog_id + version + digest + bounded content` 固定到 Durable Run。后续 phase 使用该 Run snapshot，而不是重新解析当前 Market 或安装目录。Skill 更新不能改变进行中的 prompt，Tool/Capability 撤销仍立即按当前 runtime policy 失败关闭。
...
- 对 Skill 只输出稳定 metadata 与 availability；digest 和正文由相关 turn 的 Skill loader 在固定 Run snapshot 时处理。
...
- 新增 Instruction Skill 不会自动新增系统能力或 capability ontology 类目；它通过独立 Skill Registry 投影 metadata。
...
Skill 的包格式、安装生命周期和上下文边界见 [Agent Skills](/agent/skills.md)。
