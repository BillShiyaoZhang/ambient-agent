...
### C. Data 层 (Model)
- **局部临时状态 (Local State)**: 使用 `ambient.state` 进行管理，适用于跨组件的轻量级状态传递和单页布局的数据存取。
- **持久化图数据 (Graph Database)**: Widget 的长期业务数据全部托管在后端的 SQLite 图数据库中。Widget 内部通过调用 `ambient.graph.subscribe` 声明式地监听目标 Schema，或者通过 `ambient.graph.mutate` 提交原子事务变更。
