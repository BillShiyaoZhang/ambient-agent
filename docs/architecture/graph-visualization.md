...
- 按节点类型显隐，选中节点后聚焦一跳邻居；
- 水平/垂直布局切换与一键恢复完整拓扑；
- 在节点外的画布空白区域按下并拖动时平移整个视图，节点拖动仍只移动所选节点；
- 提供带 accessible name 的全屏进入/退出控件；全屏模式保留全部探索能力，将键盘焦点约束在图谱内，并可通过控件或 `Escape` 退出；`Escape` 只退出图谱全屏，不同时关闭承载它的对话框或抽屉；
- 节点和边的详情面板、图例、空状态、错误状态和数据截断提示；
- 状态除颜色外还使用文字/图标表达，并遵循 Host 的亮色、暗色与 reduced-motion 设置；
- 窄屏时画布与详情面板纵向排列，所有控件有 accessible name；
- 所有由共享组件生成的标题、控件、状态、提示、详情字段和无障碍名称都支持中文与英文，并随 Host 当前语言即时切换；业务数据原文与稳定的图身份不因翻译改变。

各场景适配器遵循同一语言边界：Schema、Agent 工作流和隐私数据地图适配器生成的标题、说明、回退名称、摘要、详情字段名、badge、动作、已知边标签、覆盖范围、盲区与限制均按 Host 语言生成；Ontology/KG 的已知快照标题与说明也由 Host 本地化。实体 ID、应用名、provider、record 标签、运行摘要/错误、后端诊断和其他业务值必须保留原文。Host 切换语言时使用已保留的原始响应重新投影当前场景，不重新请求后端，也不改变节点/边 ID。

自动布局先把图划分为互不连通的弱连通分量，再分别计算层级与排布。不同分量使用互不重叠、带明确间距的区域，不能把无关联节点穿插进同一个层级序列。每个分量内，同一逻辑层级的节点尽可能共享精确的层坐标：从左到右时位于同一列，从上到下时位于同一行；环与多父节点使用确定性的分量压缩和最短层级规则，避免因输入顺序产生跳列或跳行。适应视图与恢复操作必须覆盖全部分量。
...
- 用户可以通过空白画布拖动来平移整个视图，也可以缩放、拖动节点、搜索、过滤、切换布局、聚焦邻居并检查节点/边详情。
- 用户可以进入和退出全屏模式；全屏中保留所有交互，并提供无障碍名称和 `Escape` 退出路径。
- 自动布局让同一层级共享行/列，并把无关联的图分量放在互不重叠的独立区域。
- 图探索器及所有接入场景的共享 UI 支持中文与英文，并随 Host 语言切换，不改变业务数据与图身份。
