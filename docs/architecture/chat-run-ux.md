...
> 状态：讨论稿。本文只定义信息架构、事件契约和实施顺序，不在本轮直接改动前端视觉。
...
1. Run 完成后默认折叠过程，只展开最终回答（推荐）。
2. 工具默认显示人类可读动作，原始命令只在详情/debug 中显示（推荐）。
3. 桌面聊天宽度从 380 px 调到约 420–440 px，以容纳 Run 卡；移动端保持全屏 drawer（推荐）。
4. 低风险 plan/schema 选择内联，高风险权限仍使用 blocking dialog（推荐）。
